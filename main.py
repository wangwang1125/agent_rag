...
from chat import get_model_response, get_unified_response
    ...
    with gr.Blocks(theme=gr.themes.Base(),css=".gradio_container { background-color: #f0f0f0; }") as chat:
        ...
        with gr.Row():     
            ...
            with gr.Column(scale=5):
                ...
                with gr.Accordion(label="工作模式设置",open=True):
                    work_mode = gr.Radio(choices=['rag', 'multi_agent'], label="工作模式", value="rag", interactive=True)
                    gr.HTML("<p><b>RAG模式</b>：基于知识库检索增强生成<br><b>多智能体模式</b>：基于医疗知识库的智能健康分析，支持症状分析、异常诊断等专业功能</p>")
        ...
        input_message.submit(fn=user,inputs=[input_message,chatbot],outputs=[input_message,chatbot],queue=False).then(
            fn=get_unified_response,inputs=[input_message,chatbot,work_mode,model,temperature,max_tokens,history_round,knowledge_base,similarity_threshold,chunk_cnt],outputs=[chatbot,chunk_text]
            )
